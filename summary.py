...
def compute_speedups(benchmarks, reference_avg):
    for b in benchmarks:
        b['speedup'] = b['avg'] / reference_avg if b['avg'] > 0 else float('inf')
...
def save_to_csv(benchmarks, output_file):
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['WMS', 'Container', 'AvgTime', 'StdDev', 'Speedup'])
        for b in benchmarks:
            writer.writerow([b['wms'], b['container'], f"{b['avg']:.4f}", f"{b['stdev']:.4f}", f"{b['speedup']:.4f}"])
...
def save_graphs(benchmarks, output_dir):
    ...
    plt.figure(figsize=(12, 6))
    plt.bar(labels, speedups, color='green')
    plt.ylabel('Speedup (relative to Native+Local)')
    plt.title('Benchmark Speedups')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(output_dir / 'speedups.png')
    plt.close()
...
def main():
    ...
    compute_speedups(benchmarks, reference_avg)
